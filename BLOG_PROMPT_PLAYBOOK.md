...
用於生成 LemonKAI / KAI K8s之路文章。目標不是 AI 摘要，而是 KAI 自己的 infra notebook：有理解、有故事線、有可帶走的心智模型。
...
2. 一個能放進自己腦子的心智模型
3. 一條能和前後文章串起來的故事線
4. 幾個真的能拿去排查/設計/思考的實用點

## KAI Blog Writing Constitution v2

### What makes it KAI

KAI 的文章不是「完整介紹某概念」，而是「把一個問題放進自己的技術路線裡理解」。
...
KAI K8s 主線文章必須知道自己在系列裡的位置：

- **上一章留下什麼問題**
- **這一章解哪個核心卡點**
- **下一章自然接去哪裡**

文章不要像孤立知識卡片，要像一條慢慢長出來的 Kubernetes 心智路線。
...
Core writing goal:
Do not merely summarize the topic. Build a mental model the reader can remember. The post should feel like one chapter in a longer Kubernetes learning story.
...
3. Mental model: explain the concept using a clear metaphor or boundary.
4. Technical breakdown: explain what actually happens in Kubernetes.
5. Practical lens: show how this affects debugging, design, or operations.
6. Common trap: name the misconception engineers often have.
7. Kai's bias / field note: add a short personal conclusion or rule of thumb.
8. Bridge: end by connecting this chapter to the next concept in the series.
...
- IG image prompt using the correct visual lane:
  ...
  - standalone concept -> Concept artifact
- quality_score
- needs_fact_check
```
...
This is part of a long-running Kubernetes learning path. The post must help readers build a durable mental model, not just learn a definition.
...
- Make the concept memorable with one central metaphor.
...
3. The mental model
4. What Kubernetes is actually doing
5. What beginners usually misunderstand
6. How I would inspect/debug it
...
Tone:
Clean, direct, personal, technical. No hype. No textbook voice.
```
...
- story chain: does it connect to the broader K8s path?
